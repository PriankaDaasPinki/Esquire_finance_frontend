...
const CoaListComponent = ({ apiEndpoint, detailsURL }) => {
  ...
  const handleSelect = (selectedKeys, info) => {
    ...
    console.log('handleSelectLoading start',handleSelectLoading);  // Debugging log
  
    const selectedNode = info.node;
  
    if (selectedNode) {
      // Simulate an async operation to mimic fetching or processing
      setTimeout(() => {
        const nodeData = data.find(item => item.id === selectedNode.key);
  
        setHandleSelectLoading(false);  // Stop loading once the search is done
        console.log("handleSelectLoading stopped",handleSelectLoading);  // Debugging log
  
        if (nodeData) {
          const parentData = nodeData.parent_id
            ? data.find((fData) => fData.id === nodeData.parent_id)
            : null;
  
          const parentName = parentData ? {
            value: parentData.id,
            label: parentData.account_name,
          } : null;
  
          // Update form data and selected parent name
          setFormData({
            p_id: nodeData.id,
            head_label: nodeData.head_level,
            p_account_name: createFormOpen ? '' : nodeData.account_name,
      ...
      }, 500);  // Simulate some delay to show the loading spinner
    }
  };
